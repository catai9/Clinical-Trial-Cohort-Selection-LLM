...
from few_shot_prompts import mi_6mos_2018_examples

TASK_MI_6MOS = 'MI-6MOS'

@register_task(name='MI-6MOS')
class MI6Mos(n2c22018Task):
    """
    MI-6MOS
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_MI_6MOS]['few_shot']
    ...
    def filter_records_6months(self, text, text_id):
        """
        Split the text into different records 
        Keep only texts from less than six months
        """
        ...
        if records[-1][0] == 'unknown_date':
            ...
            # 3. Get all texts < 6 months
            for i in range(-min(6, len(records)), -1, 1):
                ## if the date is less than 6 months before the last date (last record)
                ## then keep the text
                ## otherwise, skip
                date = records[i][0]
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        ## Split the text into different records 
        record_texts = self.filter_records_6months(text, text_id)
        ...
        if self.few_shot:
            preamble = mi_6mos_2018_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_MI_6MOS} is " + '\n'.join(preamble))
...
Question: Does the text mention that the patient recently had MI?
