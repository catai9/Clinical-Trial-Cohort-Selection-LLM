...
from few_shot_prompts import intuitive_CAD_2008_examples

TASK_INTUITIVE_CAD = 'intuitive_CAD'

@register_task(name='intuitive_CAD')
class IntuitiveCAD(n2c22018Task):
    """
    intuitive_CAD
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_INTUITIVE_CAD]['few_shot']
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        if self.few_shot:
            preamble = intuitive_CAD_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_INTUITIVE_CAD} is " + '\n'.join(preamble))
