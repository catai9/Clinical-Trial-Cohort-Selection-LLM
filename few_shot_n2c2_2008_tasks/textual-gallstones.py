...
from few_shot_prompts import textual_gallstones_2008_examples

TASK_TEXTUAL_GALLSTONES = 'textual_Gallstones'

@register_task(name='textual_Gallstones')
class TextualGallstones(n2c22018Task):
    """
    textual_Gallstones
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_TEXTUAL_GALLSTONES]['few_shot']
    ...
    def get_task_name(self):
        ...
        """
        return TASK_TEXTUAL_GALLSTONES
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        if self.few_shot:        
            preamble = textual_gallstones_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_TEXTUAL_GALLSTONES} is " + '\n'.join(preamble))
