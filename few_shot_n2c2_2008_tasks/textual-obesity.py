...
from few_shot_prompts import textual_obesity_2008_examples

TASK_TEXTUAL_OBESITY = 'textual_Obesity'

@register_task(name='textual_Obesity')
class TextualObesity(n2c22018Task):
    """
    textual_Obesity
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_TEXTUAL_OBESITY]['few_shot']
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        sorted_kept_passages = retriever.get_best_passages((text_id, sentences),
                                                           (TASK_TEXTUAL_OBESITY, task_query),
                                                           max_passages=5,
                                                           elbow_cutoff=True,
        ...
        if self.few_shot:        
            preamble = textual_obesity_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_TEXTUAL_OBESITY} is " + '\n'.join(preamble))
