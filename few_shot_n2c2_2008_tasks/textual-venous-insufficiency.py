...
from few_shot_prompts import textual_venous_insufficiency_2008_examples

TASK_TEXTUAL_VENOUS_INSUFFICIENCY = 'textual_Venous Insufficiency'

@register_task(name='textual_Venous Insufficiency')
class TextualVenousInsufficiency(n2c22018Task):
    """
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_TEXTUAL_VENOUS_INSUFFICIENCY]['few_shot']
    ...
    def get_task_name(self):
        ...
        """
        return TASK_TEXTUAL_VENOUS_INSUFFICIENCY
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        sorted_kept_passages = retriever.get_best_passages((text_id, sentences),
                                                           (TASK_TEXTUAL_VENOUS_INSUFFICIENCY, task_query),
                                                           max_passages=5,
                                                           elbow_cutoff=True,
        ...
        if self.few_shot:    
            preamble = textual_venous_insufficiency_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_TEXTUAL_VENOUS_INSUFFICIENCY} is " + '\n'.join(preamble))
