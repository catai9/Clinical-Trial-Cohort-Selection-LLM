...
from few_shot_prompts import textual_CAD_2008_examples

TASK_TEXTUAL_CAD = 'textual_CAD'

@register_task(name='textual_CAD')
class TextualCAD(n2c22018Task):
    """
    textual_CAD
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_TEXTUAL_CAD]['few_shot']
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        if self.few_shot:        
            preamble = textual_CAD_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_TEXTUAL_CAD} is " + '\n'.join(preamble))
    ...
    Context: "{}"
            ...
            # Here, the answer ENDS WITH "Yes" or "No", because the answer is at the end
            # of the chain of thoughts
            answer = answer.lower()
...
Question: Does the text mention that the patient has atherosclerotic CV disease (CAD)?
