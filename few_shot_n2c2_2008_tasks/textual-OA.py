...
from few_shot_prompts import textual_OA_2008_examples

TASK_TEXTUAL_OA = 'textual_OA'

@register_task(name='textual_OA')
class TextualOA(n2c22018Task):
    """
    textual_OA
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_TEXTUAL_OA]['few_shot']
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        if self.few_shot:        
            preamble = textual_OA_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_TEXTUAL_OA} is " + '\n'.join(preamble))
