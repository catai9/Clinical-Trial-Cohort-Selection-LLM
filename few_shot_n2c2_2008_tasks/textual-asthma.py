...
from few_shot_prompts import textual_asthma_2008_examples

TASK_TEXTUAL_ASTHMA = 'textual_Asthma'

@register_task(name='textual_Asthma')
class TextualAsthma(n2c22018Task):
    """
    textual_Asthma
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_TEXTUAL_ASTHMA]['few_shot']
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        sorted_kept_passages = retriever.get_best_passages((text_id, sentences),
                                                           (TASK_TEXTUAL_ASTHMA, task_query),
                                                           max_passages=5,
                                                           elbow_cutoff=True,
        ...
        if self.few_shot:      
            preamble = textual_asthma_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_TEXTUAL_ASTHMA} is " + '\n'.join(preamble))
