...
from few_shot_prompts import textual_gout_2008_examples

TASK_TEXTUAL_GOUT = 'textual_Gout'

@register_task(name='textual_Gout')
class TextualGout(n2c22018Task):
    """
    textual_Gout
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_TEXTUAL_GOUT]['few_shot']
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        if self.few_shot:        
            preamble = textual_gout_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_TEXTUAL_GOUT} is " + '\n'.join(preamble))
