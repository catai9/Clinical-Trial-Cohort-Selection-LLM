...
from few_shot_prompts import textual_OSA_2008_examples

TASK_TEXTUAL_OSA = 'textual_OSA'

@register_task(name='textual_OSA')
class TextualOSA(n2c22018Task):
    """
    textual_OSA
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_TEXTUAL_OSA]['few_shot']
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        if self.few_shot:        
            preamble = textual_OSA_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_TEXTUAL_OSA} is " + '\n'.join(preamble))
