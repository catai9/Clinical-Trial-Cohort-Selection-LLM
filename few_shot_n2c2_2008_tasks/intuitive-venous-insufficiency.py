...
from few_shot_prompts import intuitive_venous_insufficiency_2008_examples

TASK_INTUITIVE_VENOUS_INSUFFICIENCY = 'intuitive_Venous Insufficiency'

@register_task(name='intuitive_Venous Insufficiency')
class IntuitiveVenousInsufficiency(n2c22018Task):
    """
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_INTUITIVE_VENOUS_INSUFFICIENCY]['few_shot']
    ...
    def get_task_name(self):
        ...
        """
        return TASK_INTUITIVE_VENOUS_INSUFFICIENCY
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        if self.few_shot: 
            preamble = intuitive_venous_insufficiency_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_INTUITIVE_VENOUS_INSUFFICIENCY} is " + '\n'.join(preamble))
...
Question: Does the text mention that the patient has venous insufficiency?
        """.format(abstract.rstrip()).strip()
            ...
            if self.logger is not None:
                self.logger.debug('\nANSWER: ' + answer)
            answer = answer.lower()
            if answer.startswith('yes') or answer.endswith('yes'):
                return 'Y'
            else:
                return 'N'
