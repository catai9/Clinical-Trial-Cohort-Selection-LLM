...
from few_shot_prompts import textual_PVD_2008_examples

TASK_TEXTUAL_PVD = 'textual_PVD'

@register_task(name='textual_PVD')
class TextualPVD(n2c22018Task):
    """
    textual_PVD
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_TEXTUAL_PVD]['few_shot']
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        if self.few_shot:        
            preamble = textual_PVD_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_TEXTUAL_PVD} is " + '\n'.join(preamble))
