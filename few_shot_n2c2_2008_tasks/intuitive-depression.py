...
from few_shot_prompts import intuitive_depression_2008_examples

TASK_INTUITIVE_DEPRESSION = 'intuitive_Depression'

@register_task(name='intuitive_Depression')
class IntuitiveDepression(n2c22018Task):
    """
    intuitive_Depression
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_INTUITIVE_DEPRESSION]['few_shot']
    ...
    def get_task_name(self):
        ...
        """
        return TASK_INTUITIVE_DEPRESSION
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        sorted_kept_passages = retriever.get_best_passages((text_id, sentences),
                                                           (TASK_INTUITIVE_DEPRESSION, task_query),
                                                           max_passages=5,
                                                           elbow_cutoff=True,
        ...
        if self.few_shot: 
            preamble = intuitive_depression_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_INTUITIVE_DEPRESSION} is " + '\n'.join(preamble))
