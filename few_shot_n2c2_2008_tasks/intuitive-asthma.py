...
from few_shot_prompts import intuitive_asthma_2008_examples

TASK_INTUITIVE_ASTHMA = 'intuitive_Asthma'

@register_task(name='intuitive_Asthma')
class IntuitiveAsthma(n2c22018Task):
    """
    intuitive_Asthma
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_INTUITIVE_ASTHMA]['few_shot']
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        sorted_kept_passages = retriever.get_best_passages((text_id, sentences),
                                                           (TASK_INTUITIVE_ASTHMA, task_query),
                                                           max_passages=5,
                                                           elbow_cutoff=True,
        ...
        if self.few_shot:        
            preamble = intuitive_asthma_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_INTUITIVE_ASTHMA} is " + '\n'.join(preamble))
...
Question: Does the text mention that the patient has asthma?
