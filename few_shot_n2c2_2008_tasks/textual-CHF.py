...
from few_shot_prompts import textual_CHF_2008_examples

TASK_TEXTUAL_CHF = 'textual_CHF'

@register_task(name='textual_CHF')
class TextualCHF(n2c22018Task):
    """
    textual_CHF
    ...
    def __init__(self, config):
        ...
        self.few_shot = self.config[TASK_TEXTUAL_CHF]['few_shot']
    ...
    def __call__(self, text_id, text, retriever, fastchat, preamble_type):
        ...
        if self.few_shot:   
            preamble = textual_CHF_2008_examples.example_dictionary[preamble_type]
            fastchat.init_conversation(preamble)
            
            if self.logger is not None:
                self.logger.debug(f"The preamble conversation for {TASK_TEXTUAL_CHF} is " + '\n'.join(preamble))
